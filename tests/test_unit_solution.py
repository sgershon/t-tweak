...
# ---------------------------------------------------------------------------
@pytest.mark.parametrize(
    "fake_date, expected_weekday",
    [
        (datetime(2024, 1, 1), "Mon"),
        (datetime(2024, 1, 2), "Tue"),
        (datetime(2024, 1, 3), "Wed"),
        (datetime(2024, 1, 4), "Thu"),
        (datetime(2024, 1, 5), "Fri"),
        (datetime(2024, 1, 6), "Sat"),
        (datetime(2024, 1, 7), "Sun"),
    ]
)
def test_server_time_weekdays(monkeypatch, fake_date, expected_weekday):

    # Monkeypatch the network time provider
    monkeypatch.setattr(extra, "get_network_time", lambda: fake_date)

    # server_time() raises HTTPException(203)
    with pytest.raises(HTTPException) as excinfo:
        main.server_time()

    err = excinfo.value
    assert err.status_code == 203

    # Extract the returned string
    result = err.detail

    # Check that the weekday is correct
    assert expected_weekday in result
...
def test_server_time_client(monkeypatch):
    # Start from an arbitrary "today"
    base_date = datetime(2024, 1, 1)

    # Monkeypatch get_network_time to return base_date
    monkeypatch.setattr(extra, "get_network_time", lambda: base_date)

    # Call the API once to get the weekday for the base date
    r = client.get("/time")
    assert r.status_code == 203
    first_weekday = r.json()["detail"][:3]   # e.g. "Mon", "Tue", etc.

    # A helper list to check weekday progression
    weekdays = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
    start_index = weekdays.index(first_weekday)

    # Now test the next 100 days
    for i in range(1, 101):
        fake_date = base_date + timedelta(days=i)

        # Update monkeypatch to return the new date
        monkeypatch.setattr(extra, "get_network_time", lambda d=fake_date: d)

        r = client.get("/time")
        assert r.status_code == 203

        returned_weekday = r.json()["detail"][:3]

        # Expected weekday cycles every 7 days
        expected_weekday = weekdays[(start_index + i) % 7]

        assert returned_weekday == expected_weekday
